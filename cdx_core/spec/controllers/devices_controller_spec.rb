...
describe DevicesController do
  ...
  let!(:site)              { institution.sites.make time_zone: 'Tokyo' }
  ...
  context 'Index' do
    ...
    it 'should download all pages' do
      devices = 30.times.map do |i|
        Device.make institution: institution, site: site, device_model: device_model
      end
      get :index, format: :csv
  ...
  context 'New' do
    ...
    it 'loads published device models and from allowed institutions' do
      ...
      DeviceModel.make(:unpublished, institution: Institution.make)
    ...
    it 'loads unpublished device models only of context institution' do
      published = device_model
      institution.device_models.make(:unpublished)
    ...
    it 'loads all sites from allowed institutions in context' do
      ...
      yet_another_institution.sites.make
    ...
    context 'when context is site' do
      ...
      it 'should initialize device with site timezone' do
        expect(assigns(:device).time_zone).to eq(site.time_zone)
      end
    end
  end
  ...
  context 'Edit' do
    ...
    it 'loads published device models and from device institution' do
      ...
      other_user_institution.device_models.make(:unpublished)
  ...
  describe 'generate_activation_token' do
    before do
      post :generate_activation_token, institution_id: device.institution_id, id: device.id, format: :json
      device.reload
    end

    it { expect(device.activation_token).not_to be_nil }
  end
