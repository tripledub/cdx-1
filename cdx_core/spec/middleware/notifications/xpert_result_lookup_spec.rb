...
describe Notifications::XpertResultLookup do
  ...
  let!(:notification_on_detected_mtb)      { Notification.make(institution: patient.institution, detection: 'mtb') }
  let!(:notification_on_detected_rif)      { Notification.make(institution: patient.institution, detection: 'rif') }
  ...
  describe '#check_notifications' do
    context 'when result is the same patient and site, no mtb or rif' do
      before do
        lookup_no_detection.check_notifications
      end
    ...
    context 'when result is the same patient and site, mtb and rif detected' do
      before do
        lookup_tb_and_rif.check_notifications
      end
