...
RSpec.describe PatientResults::Finder do
  ...
  let!(:test_result)       { XpertResult.make encounter: encounter, institution: institution, device: device, created_at:  3.days.ago.strftime('%Y-%m-%d') }
  let!(:test_result2)      { MicroscopyResult.make encounter: encounter, institution: institution, device: device2, created_at: 16.days.ago.strftime('%Y-%m-%d') }
  let!(:test_result3)      { DstLpaResult.make encounter: encounter2, institution: institution, device: device3, created_at: 28.days.ago.strftime('%Y-%m-%d'), sample_identifier: sample_identifier }
  let!(:test_result4)      { CultureResult.make encounter: encounter3, institution: institution, device: device, created_at: 53.days.ago.strftime('%Y-%m-%d') }
