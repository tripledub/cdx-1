    ...
    it 'should return elements formated' do
      expect(CSV.parse(described_class.csv_query(PatientResult.all))[1]).to eq(
        [
          ...
          Extras::Dates::Format.datetime_with_time_zone(CultureResult.first.sample_collected_at, :full_time_with_timezone),
          CultureResult.first.examined_by,
          Extras::Dates::Format.datetime_with_time_zone(CultureResult.first.result_at, :full_time_with_timezone),
