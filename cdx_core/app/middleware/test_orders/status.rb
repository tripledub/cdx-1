    ...
    class << self
      ...
      def update_status(encounter)
        ...
        if all_finished?(encounter)
          return update_and_log(encounter, 'closed')
        elsif any_pending_approval?(encounter)
          return update_and_log(encounter, 'in_progress')
        elsif any_sample_received?(encounter)
          return update_and_log(encounter, 'samples_received')
        elsif any_sample_collected?(encounter)
          return update_and_log(encounter, 'samples_collected')
        elsif order_is_pending?(encounter)
          return update_and_log(encounter, 'pending')
        end
      end
