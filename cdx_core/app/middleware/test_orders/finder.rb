  ...
  class Finder
    ...
    def init_query
      @filter_query = Encounter.joins('LEFT OUTER JOIN sites as performing_sites ON performing_sites.id=encounters.performing_site_id')
                               ...
                               .includes(:institution, :patient, :user, :site, :performing_site, :samples, :patient_results)
    end
    ...
    def filter_by_navigation_context
      ...
      if @navigation_context.exclude_subsites && @navigation_context.site
        @filter_query = filter_query.where(
          'sites.id = ? OR encounters.performing_site_id = ?',
          @navigation_context.site.id,
          @navigation_context.site.id
        )
        ...
        @filter_query = filter_query.where(
          'sites.id = ? OR sites.id IN (?) OR encounters.performing_site_id = ?',
          @navigation_context.site.id,
          subsites,
          @navigation_context.site.id
        )
