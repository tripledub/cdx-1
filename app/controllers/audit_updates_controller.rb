class AuditUpdatesController < ApplicationController
  ...
  expose(:audit_updates, scope: -> { @audit_log.audit_updates })

  def index
    render json: Presenters::AuditUpdates.patient_view(audit_updates.order(set_order_from_params).limit(30).offset(params[:page] || 0))
  end
  ...
  def find_patient_and_log
    ...
    @audit_log = patient.audit_logs.where(id: params[:patient_log_id])
  end
