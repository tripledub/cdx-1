module Reports
  class Site < Base
    ...
    def generate_chart
      ...
      sites.each do | site |
        found_data.each do | found_site_data |
          data << { label: site[0], y: found_site_data[:count] } if site_is_present?(found_site_data, site[0])
        end
      end
    ...
    def site_is_present?(found_site_data, site)
      found_site_data[:uuid] && site && site.include?(found_site_data[:site_name])
    end
  end
end
